...
use rsmgclient::FunctionType;
use rsmgclient::Gromet;
use std::env;
use std::fs;
...
fn execute_query(query: &str) -> Result<(), MgError> {
    // Connect to Memgraph.
...
fn create_function_net(gromet: &Gromet, mut start: u32) -> Vec<String> {
    ...
    for boxf in gromet.r#fn.bf.as_ref().unwrap().iter() {
        ...
        match boxf.function_type {
            FunctionType::Expression => {
                ...
                let eboxf = gromet.attributes[idx as usize];
                // construct opo nodes, if not none
                if !eboxf.value.opo.is_none() {
                    // grab name which is one level up and based on indexing because most frustrating that way
                    let mut opo_name = String::from("un-named");
                    for port in gromet.r#fn.pof.unwrap().iter() {
                        if port.r#box == bf_counter {
                            if !port.name.is_none() {
                                opo_name = port.name.unwrap();
                            }
                        }
                    }
                    let mut oport: u32 = 1;
                    for op in eboxf.value.opo.as_ref().unwrap().iter() {
                        let n2 = Node {
                            n_type: String::from("Opo"),
                            value: None,
                            name: Some(opo_name),
                            node_id: format!("n{}", start),
                            out_idx: Some([oport].to_vec()),
                            in_indx: None,
                            contents: idx + 1,
                        };
                        nodes.push(n2);
                        ...
                        let e3 = Edge {
                            src: n1.node_id,
                            tgt: n2.node_id,
                            e_type: String::from("Of"),
                            prop: None,
                        };
                        egdes.push(e3);
                        // construct any metadata edges
                        if !eboxf.value.opo.unwrap()[oport as usize]
                            .metadata
                            .as_ref()
                            .is_none()
                        {
                            let e4 = Edge {
                                ...
                                tgt: String::from("meta"),
                                e_type: String::from("Metadata"),
                                prop: eboxf.value.opo.unwrap()[oport as usize].metadata,
                            };
                            egdes.push(e4);
                        } else {
                        }
                ...
                if !eboxf.value.opi.is_none() {
                    let mut iport: u32 = 1;
                    for op in eboxf.value.opi.as_ref().unwrap().iter() {
                        let n2 = Node {
                        ...
                        if !eboxf.value.opi.unwrap()[iport as usize]
                            .metadata
                            .as_ref()
                            .is_none()
                        {
                            let e4 = Edge {
                                ...
                                tgt: String::from("meta"),
                                e_type: String::from("Metadata"),
                                prop: eboxf.value.opi.unwrap()[iport as usize].metadata,
                            };
                            egdes.push(e4);
                        } else {
                        }
                ...
                // not to construct the nodes inside the expression
                let mut box_counter: u8 = 1;
                for sboxf in eboxf.value.bf.as_ref().unwrap().iter() {
                    match sboxf.function_type {
                        FunctionType::Literal => {
                            // first find the pof's for box
                            let mut pof: Vec<u32> = vec![];
                            if !eboxf.value.pof.is_none() {
                                let mut po_idx: u32 = 1;
                                for port in eboxf.value.pof.unwrap().iter() {
                            ...
                            if !eboxf.value.pif.is_none() {
                                let mut pi_idx: u32 = 1;
                                for port in eboxf.value.pif.unwrap().iter() {
                                    if port.r#box == box_counter {
                                        pif.push(pi_idx);
                                    } else {
                                    }
                            ...
                            let n3 = Node {
                                ...
                                value: Some(format!("{:?}", sboxf.value.unwrap())),
                                name: None,
                                node_id: format!("n{}", start),
                                out_idx: Some(pof),
                                in_indx: Some(pif),
                            ...
                            if (idx + 1) == node.contents {
                                ...
                                if !node.in_indx.is_none() {
                                    ...
                                    if node.n_type != "Opi" {
                                        ...
                                        for p in node.in_indx.unwrap().iter() {
                                            // push the src first, being pif
                                            if (wire.src as u32) == *p {
                                                wfopi_src_tgt.push(node.node_id);
                                            }
                                        }
                                    }
                                }
                    ...
                    for wire in eboxf.value.wfopo.unwrap().iter() {
                        ...
                        for node in nodes.iter() {
                            ...
                            if (idx + 1) == node.contents {
                                ...
                                if node.n_type == "Opo" {
                                    // iterate through port to check for tgt
                                    for p in node.out_idx.unwrap().iter() {
                                        // push the src first, being pif
                                        if (wire.src as u32) == *p {
                                            wfopo_src_tgt.push(node.node_id);
                                        }
                                    }
                                }
                            }
    ...
    for wire in gromet.r#fn.wff.unwrap().iter() {
        let src_idx = wire.src;
        let tgt_idx = wire.tgt;
        let src_id = gromet.r#fn.pif.unwrap()[src_idx as usize].id.unwrap();
        let src_box = gromet.r#fn.pif.unwrap()[src_idx as usize].r#box;
        let src_contents = gromet.r#fn.bf.unwrap()[src_box as usize].contents.unwrap();
        let tgt_id = gromet.r#fn.pof.unwrap()[tgt_idx as usize].id.unwrap();
        let tgt_box = gromet.r#fn.pof.unwrap()[tgt_idx as usize].r#box;
        let tgt_contents = gromet.r#fn.bf.unwrap()[tgt_box as usize].contents.unwrap();
    ...
    for edge in egdes.iter() {
        let edge_query = format!(
            "{} ({})-[{}:{}]->({})",
            create, edge.src, edge.src, edge.e_type, edge.tgt
        );
        queries.push(edge_query);

        if !edge.prop.is_none() {
            let set_query = format!("SET {}.relationship={}", edge.src, edge.prop.unwrap());
...
fn parse_gromet_queries(gromet: Gromet) -> Vec<String> {
    let mut queries: Vec<String> = vec![];
...
// I think how I want to run this is I create a query list and I run through the query list
fn main() {
